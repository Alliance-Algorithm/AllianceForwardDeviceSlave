...
__ALIGN_BEGIN static uint8_t USBD_AUDIO_CfgDesc[USB_AUDIO_CONFIG_DESC_SIZ] __ALIGN_END =
    {
        ...
        USBD_EP_TYPE_ISOC,                 /* bmAttributes */
        AUDIO_PACKET_SZE(USBD_AUDIO_FREQ), /* wMaxPacketSize in time (Freq(Samples)*2(Stereo)*2(HalfWord)) */
        AUDIO_FS_BINTERVAL,                /* bInterval */
                                           /* 07 byte*/
};
...
static uint8_t USBD_AUDIO_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
  ...
  (void)USBD_LL_PrepareReceive(pdev, AUDIOOutEpAdd, haudio->rx_buffer,
                               AUDIO_OUT_PACKET);
  // USBD_LL_Transmit(&hUsbDeviceFS, AUDIO_IN_EP, haud->tx_buffer, AUDIO_OUT_PACKET);
...
static uint8_t USBD_AUDIO_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  //                        &haudio->tx_buffer[haudio->tx_ptr],
  //                        haudio->tx_size);
  // haudio->tx_ptr = 0;
  if (epnum == (AUDIOInEpAdd & 0xFu))
  {
    USBD_LL_Transmit(pdev, epnum, &(haudio->tx_buffer[haudio->tx_ptr]), AUDIO_OUT_PACKET);
  }
...
void USBD_AUDIO_Sync(USBD_HandleTypeDef *pdev, AUDIO_OffsetTypeDef offset)
{
  ...
  {
    ((USBD_AUDIO_ItfTypeDef *)pdev->pUserData[pdev->classId])->AudioCmd(&haudio->rx_buffer[0], BufferSize, AUDIO_CMD_PLAY);
...
static uint8_t USBD_AUDIO_IsoINIncomplete(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  if (epnum == (AUDIOInEpAdd & 0xFu))
  {
    ...
    // haudio->tx_ptr += AUDIO_OUT_PACKET;
  }
  USBD_LL_Transmit(pdev, epnum, haudio->tx_buffer, AUDIO_OUT_PACKET);
  return (uint8_t)USBD_OK;
}
