 ...
 * @{
 */
uint8_t Rx_buf[AUDIO_TOTAL_BUF_SIZE];

static int8_t
AUDIO_Init_FS(uint32_t AudioFreq, uint32_t Volume, uint32_t options);
...
static int8_t AUDIO_PeriodicTC_FS(uint8_t *pbuf, uint32_t size, uint8_t cmd)
{
  ...
  memcpy(Rx_buf + haud->wr_ptr, pbuf, size);
  /* USER CODE BEGIN 5 */
  UNUSED(pbuf);
  UNUSED(size);
  UNUSED(cmd);
...
void Receive_FS(uint8_t *buffer, uint32_t *size)
{
  ...
  (void)USBD_LL_PrepareReceive(&hUsbDeviceFS, AUDIO_OUT_EP,
                               haud->rx_buffer,
                               AUDIO_OUT_PACKET);
}
void Transmit_FS(uint8_t *buffer, uint32_t size)
{
  ...
  haud->tx_size = size;
  haud->tx_ptr = 0;
  // USBD_LL_Transmit(&hUsbDeviceFS, AUDIO_IN_EP, &haud->tx_buffer[haud->tx_ptr], AUDIO_OUT_PACKET);
  // while (haud->tx_ptr < AUDIO_TOTAL_BUF_SIZE)
  // {
  // }
  // haud->tx_ptr = 0;
}
